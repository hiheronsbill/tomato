 ...
 * @Desc TODO
 * @Author zhxy
 * @Date 2021/9/25 11:23
 * @Version V1.0
 **/
public class MyRollingFileAppender<E> extends FileAppender<E> {
    ...
    File versionFile;
    String versionFileName = "init-version.log";
    ...
    public void start() {
        if (this.triggeringPolicy == null) {
            ...
            if (this.rollingPolicy == null) {
                ...
                // 初始化版本文件
                versionFile = new File(getParentPath());
    ...
    private String getVersionFileName() {
        ...
        try {
            ...
            if (files == null || files.length < 1) {
                new File(getParentPath() + versionFileName).createNewFile();
            } else {
    ...
    private String getParentPath() {
        String absolutePath = this.currentlyActiveFile.getAbsolutePath();
        String currentFile = this.currentlyActiveFile.getName();
        return absolutePath.substring(0, absolutePath.length() - currentFile.length());
    }
    ...
    protected void subAppend(E event) {
        ...
        // 获取版本文件名
        if(!versionFileName.equals(getVersionFileName())){
            this.lock.lock();
            try {
                this.closeOutputStream();
                this.attemptOpenFile();
